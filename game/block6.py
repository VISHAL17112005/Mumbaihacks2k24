...
score = 0
level = 1
missed_blocks = 0  # Track missed blocks
game_over = False
...
# Function to check AI difficulty adjustment based on score multiples and missed blocks
def check_ai_difficulty():
    global level, missed_blocks  # Add missed_blocks as global

    # Initialize the function attribute if it doesn't exist
    if not hasattr(check_ai_difficulty, 'last_adjusted_score'):
        check_ai_difficulty.last_adjusted_score = -1  # Initial value

    # Increase level based on score multiples of 100 and adjust block interval
    if score > 0 and score != check_ai_difficulty.last_adjusted_score:  # Only adjust if score changed

        if score % 100 == 0:  # Check for multiples of 100 first
            level += 3  # Increase level by 3 for multiples of 100
            check_ai_difficulty.last_adjusted_score = score  # Remember the last adjusted score

        elif score % 50 == 0 and score % 100 != 0:  # Check for multiples of 50 only if not a multiple of 100
            level += 1  # Increase level by 1 for multiples of 50
            check_ai_difficulty.last_adjusted_score = score  # Remember the last adjusted score

    # Set a maximum level cap to prevent infinite increase
    if level > 10:
        level = 10

    # Adjust level based on missed blocks; game over if missed blocks >= 3 
    if missed_blocks >= 3:
        print("Game Over! You missed too many blocks.")
        return True  # Indicate game over condition

    elif missed_blocks >= 2 and level > 1:
        level -= 1  # Decrease level by one if missed blocks are more than or equal to two 
        missed_blocks = 0  # Reset missed blocks after decreasing level

    return False  # Indicate game is still ongoing

# Function to reset game state
def reset_game():
    global score, level, blocks, game_over, missed_blocks
    score = 0
    level = 1
    missed_blocks = 0  # Reset missed blocks count
    blocks.clear()
    game_over = False
...
while running:
    ...
    if not game_over:
        ...
        game_over = check_ai_difficulty()  # Check if game over condition is met
